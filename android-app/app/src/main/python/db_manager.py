...
def get_data_dir():
    if is_android():
        try:
            ...
            data_dir = context.getExternalFilesDir(None).getAbsolutePath()
            print(f"Android数据目录: {data_dir}")
        except Exception as e:
            print(f"获取Android数据目录失败: {e}")
            import tempfile
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS memories (
                ...
                world_id INTEGER NOT NULL,
                character_id INTEGER,
                memory_category TEXT DEFAULT 'memory',
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS api_config (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                api1_key TEXT,
                api1_model TEXT DEFAULT 'ep-m-20260305201046-cbwgl',
                api2_key TEXT,
                api2_model TEXT DEFAULT 'ep-m-20260305201046-cbwgl'
            )
        ''')
        ...
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_memories_world_category ON memories(world_id, memory_category)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_memories_character_category ON memories(character_id, memory_category)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_memories_segment ON memories(world_id, segment)
        ''')
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_memories_is_new ON memories(world_id, character_id, is_new)
        ''')
        
        self._migrate_database()
    
    def _migrate_memories_table(self):
        cursor = self.conn.cursor()
        ...
        if 'memory_category' not in columns:
            ...
            try:
                ...
                print(f"重命名memories表失败: {e}")
                cursor.execute('DROP TABLE IF EXISTS memories_old')
                cursor.execute('ALTER TABLE memories RENAME TO memories_old')
            ...
            cursor.execute('''
                INSERT INTO memories (id, world_id, character_id, memory_category, memory_type, content, importance, segment, created_at)
                SELECT id, world_id, character_id, 'memory', memory_type, content, importance, segment, created_at
                FROM memories_old
            ''')
            
            cursor.execute('DROP TABLE memories_old')
            
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_memories_world_category ON memories(world_id, memory_category)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_memories_character_category ON memories(character_id, memory_category)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_memories_segment ON memories(world_id, segment)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_memories_is_new ON memories(world_id, character_id, is_new)')
            
            self.conn.commit()
            print("记忆表迁移完成")
        
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='short_term_memories'")
        if cursor.fetchone():
            print("正在迁移短期记忆表...")
            cursor.execute('''
                INSERT INTO memories (world_id, character_id, memory_category, memory_type, content, importance, is_new, source_message_ids, created_at)
                SELECT world_id, character_id, 'short_term', 'general', content, importance, 1, source_message_ids, created_at
                FROM short_term_memories
            ''')
            cursor.execute('DROP TABLE short_term_memories')
            self.conn.commit()
            print("短期记忆表迁移完成")
        
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='long_term_memories'")
        if cursor.fetchone():
            print("正在迁移长期记忆表...")
            cursor.execute('''
                INSERT INTO memories (world_id, character_id, memory_category, memory_type, content, importance, source_short_term_ids, consolidated_at, created_at)
                SELECT world_id, character_id, 'long_term', 'general', content, importance, source_short_term_ids, consolidated_at, created_at
                FROM long_term_memories
            ''')
            cursor.execute('DROP TABLE long_term_memories')
            self.conn.commit()
            print("长期记忆表迁移完成")
    ...
    def migrate_database(self):
        ...
        try:
            ...
            if 'api1_model' not in api_config_columns:
                ...
                cursor.execute('ALTER TABLE api_config ADD COLUMN api1_model TEXT DEFAULT "ep-m-20260305201046-cbwgl"')
                self.conn.commit()
                print("数据库迁移完成")
            ...
            if 'api2_model' not in api_config_columns:
                ...
                cursor.execute('ALTER TABLE api_config ADD COLUMN api2_model TEXT DEFAULT "ep-m-20260305201046-cbwgl"')
                self.conn.commit()
                print("数据库迁移完成")
    ...
    def update_world(self, world_id: int, name: str = None, background: str = None, current_date: str = None, current_time: str = None, user_health_mouth: str = None, user_health_anus: str = None, user_health_buttocks: str = None, user_health_penis: str = None, user_health_testicles: str = None, user_health_left_breast: str = None, user_health_right_breast: str = None, user_health_vagina: str = None, user_health_mouth_color: str = None, user_health_anus_color: str = None, user_health_buttocks_color: str = None, user_health_penis_color: str = None, user_health_testicles_color: str = None, user_health_left_breast_color: str = None, user_health_right_breast_color: str = None, user_health_vagina_color: str = None, user_message_count: int = None, total_seconds: int = None, locations: List[str] = None, user_location: str = None, communication_character: str = None, script_outline: str = None, script_chapters: str = None, current_chapter_index: int = None, script_enabled: bool = None, user_name: str = None, map_image: str = None):
        ...
        if updates:
            ...
            cursor = self.conn.cursor()
            cursor.execute(f'UPDATE worlds SET {", ".join(updates)} WHERE id = ?', params)
    ...
    def create_memory(self, world_id: int, memory_type: str = 'general', content: str = '', importance: int = 1, character_id: int = None, segment: int = 1, memory_category: str = 'memory', is_new: int = 1, source_message_ids: str = None, source_short_term_ids: str = None, event_type: str = None, location: str = None, event_date: str = None, event_time: str = None, present_character_ids: str = None) -> Memory:
        cursor = self.conn.cursor()
        cursor.execute('''
            INSERT INTO memories (
                world_id, character_id, memory_category, memory_type, content, importance,
                segment, is_new, source_message_ids, source_short_term_ids,
                event_type, location, event_date, event_time, present_character_ids
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (world_id, character_id, memory_category, memory_type, content, importance,
              segment, is_new, source_message_ids, source_short_term_ids,
              event_type, location, event_date, event_time, present_character_ids))
    ...
    def get_memories_by_world(self, world_id: int, memory_type: str = None, limit: int = 20, memory_category: str = None) -> List[Memory]:
        cursor = self.conn.cursor()
        conditions = ['world_id = ?']
        params = [world_id]
    ...
    def get_memories_by_character(self, character_id: int, memory_type: str = None, limit: int = 20, memory_category: str = None) -> List[Memory]:
        cursor = self.conn.cursor()
    ...
    def create_short_term_memory(self, world_id: int, content: str, importance: int = 1, character_id: int = None, source_message_ids: str = None, memory_type: str = 'general') -> Memory:
    ...
    def get_short_term_memory(self, memory_id: int) -> Optional[Memory]:
        return self.get_memory(memory_id)
    
    def get_short_term_memories(self, world_id: int, character_id: int = None, limit: int = 20) -> List[Memory]:
        cursor = self.conn.cursor()
        if character_id:
            cursor.execute('''
            ...
            cursor.execute('''
                SELECT * FROM memories 
                WHERE world_id = ? AND memory_category = 'short_term'
                ORDER BY importance DESC, created_at DESC LIMIT ?
            ''', (world_id, limit))
        rows = cursor.fetchall()
        return [Memory(*row) for row in rows]
    ...
    def get_new_short_term_memory_count(self, world_id: int, character_id: int) -> int:
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT COUNT(*) FROM memories 
            WHERE world_id = ? AND character_id = ? AND memory_category = 'short_term' AND is_new = 1
        ''', (world_id, character_id))
        result = cursor.fetchone()[0]
        return result if result is not None else 0
    
    def reset_short_term_memory_counter(self, world_id: int, character_id: int):
        cursor = self.conn.cursor()
        cursor.execute('''
            UPDATE memories SET is_new = 0 
            WHERE world_id = ? AND character_id = ? AND memory_category = 'short_term'
        ''', (world_id, character_id))
        self.conn.commit()
    ...
    def update_short_term_memory(self, memory_id: int, content: str = None, importance: int = None):
        self.update_memory(memory_id, content, importance)
    ...
    def create_long_term_memory(self, world_id: int, content: str, importance: int = 1, character_id: int = None, source_short_term_ids: str = None, memory_type: str = 'general') -> Memory:
        return self.create_memory(
            world_id=world_id,
            content=content,
            importance=importance,
            character_id=character_id,
            source_short_term_ids=source_short_term_ids,
            memory_type=memory_type,
            memory_category='long_term',
            is_new=0
        )
    
    def get_long_term_memory(self, memory_id: int) -> Optional[Memory]:
        return self.get_memory(memory_id)
    
    def get_long_term_memories(self, world_id: int, character_id: int = None, limit: int = 20) -> List[Memory]:
        cursor = self.conn.cursor()
        if character_id:
            cursor.execute('''
                SELECT * FROM memories 
                WHERE world_id = ? AND character_id = ? AND memory_category = 'long_term'
                ORDER BY importance DESC, created_at DESC LIMIT ?
            ''', (world_id, character_id, limit))
        else:
            cursor.execute('''
                SELECT * FROM memories 
                WHERE world_id = ? AND memory_category = 'long_term'
                ORDER BY importance DESC, created_at DESC LIMIT ?
            ''', (world_id, limit))
        rows = cursor.fetchall()
        return [Memory(*row) for row in rows]
    ...
    def replace_long_term_memories(self, world_id: int, character_id: int, memories_data: List[dict]):
        cursor = self.conn.cursor()
        cursor.execute('''
    ...
    def create_event(self, world_id: int, event_content: str, event_type: str, location: str, event_date: str, event_time: str, importance: int = 1, present_character_ids: str = None) -> Memory:
        return self.create_memory(
            world_id=world_id,
    ...
    def get_events(self, world_id: int) -> List[Memory]:
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT * FROM memories 
            WHERE world_id = ? AND memory_category = 'event'
    ...
    def get_memories_by_segment(self, world_id: int, segment: int, memory_type: str = None) -> List[Memory]:
        ...
        if memory_type:
            cursor.execute('''
                SELECT * FROM memories 
                WHERE world_id = ? AND segment = ? AND memory_type = ?
                ORDER BY importance DESC, created_at DESC
            ''', (world_id, segment, memory_type))
        else:
            cursor.execute('''
                SELECT * FROM memories 
                WHERE world_id = ? AND segment = ?
                ORDER BY importance DESC, created_at DESC
            ''', (world_id, segment))
        rows = cursor.fetchall()
        return [Memory(*row) for row in rows]
    ...
    def delete_memories_from_segment(self, world_id: int, from_segment: int, character_id: int = None):
        ...
        if character_id:
            cursor.execute('''
                DELETE FROM memories 
                WHERE world_id = ? AND character_id = ? AND segment >= ?
    ...
    def delete_memories_after_time(self, world_id: int, after_time: str, character_id: int = None):
        cursor = self.conn.cursor()
        if character_id:
            cursor.execute('''
                DELETE FROM memories 
                WHERE world_id = ? AND character_id = ? AND created_at >= ?
            ''', (world_id, character_id, after_time))
        else:
            cursor.execute('''
                DELETE FROM memories 
    ...
    def end_active_call(self, call_id: int):
        ...
        if call:
            ...
            now = datetime.now()
            cursor.execute('''
                INSERT INTO call_history (world_id, character_id, character_name, call_start_date, call_start_time, call_end_date, call_end_time, duration_seconds)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (call['world_id'], call['character_id'], call['character_name'], call['call_start_date'], call['call_start_time'], now.strftime('%Y-%m-%d'), now.strftime('%H:%M:%S'), 0))
        cursor.execute('DELETE FROM active_calls WHERE id = ?', (call_id,))
        self.conn.commit()
    
    def get_call_history(self, world_id: int, limit: int = 50) -> List[Dict]:
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT * FROM call_history WHERE world_id = ? ORDER BY created_at DESC LIMIT ?
        ''', (world_id, limit))
        rows = cursor.fetchall()
